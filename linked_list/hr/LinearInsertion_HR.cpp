...
MatchResult Matcher::matchCandidates(JobRole* role, Candidate* candidates, SkillNode* searchSkills) {
    ...
    size_t linearMemory = sizeof(int) * 3 + sizeof(string);
    size_t insertionMemory = sizeof(Candidate*) * 2; 

    delete[] weights;
    return { sorted, linearTime, insertionTime, linearMemory, insertionMemory};
}

void insertion_linear() {
    cout << "===============================================" << endl;
        ...
        while (!backToSearchMenu && !exitProgram) {
            ...
            while (!backAfterReport && !exitProgram) {
                ...
                if (postChoice == "1") {
                    ...
                    cout << "Linear Search Time    : " << fixed << setprecision(3) << result.searchTimeMs << " ms\n";
                    cout << "Linear Search Memory  : " << fixed << setprecision(3) << (result.searchMemoryBytes / 1024.0) << " KB\n"; 
                    cout << "Insertion Sort Time   : " << fixed << setprecision(3) << result.sortTimeMs << " ms\n";
                    cout << "Insertion Sort Memory : " << fixed << setprecision(3) << (result.sortMemoryBytes / 1024.0) << " KB\n";
